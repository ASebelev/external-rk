...
class MapData:
    def __init__(self, power=0, pressure=0, diameter=0, drkPressure=0, drkPower=0):
...
for mode in fullMode:
    ...
    listDiameter = [i for i in range(4, 51, 1)]
    listPowPress = []
    for currentDiameter in listDiameter:
        ...
        resDir = f'{globalResultDir}/freq({mode.freq})_Diameter({currentDiameter})'
        if RUN_MODE is None:
            os.mkdir(resDir)
        ...
        try:
            ...
            tmpMap = MapData(diameter=currentDiameter)
            for item in resText:
                if 'Ne' in item:
    ...
    for currentMAP in mode.mapData:
        diff = 9999999999999999999999.9

        for currentPowPres in listPowPress:
            #print(f"{currentMAP.power} - {currentPowPres.drkPower}")
            if abs(currentMAP.power - currentPowPres.drkPower) < diff and currentPowPres.diameter > backDiameter:
                #print(f"OK -> {currentMAP.power} - {currentPowPres.drkPower}")
                ...
                diff = abs(currentMAP.power - currentPowPres.drkPower)
    mode.listDRK = listPowPress
for mode in fullMode:
    with open(f'{globalResultDir}/Freq_{mode.freq}.csv', 'w') as file:
        file.write("MAP_Power;DRK_Power;MAP_Pressure;DRK-Pressure;Diameter\n")
        for currentMAP in mode.mapData:
            file.write(f"{str(currentMAP.power).replace('.',',')};{str(currentMAP.drkPower).replace('.',',')};"
                       ...
                       f"{str(currentMAP.diameter).replace('.',',')}\n")
        file.write("\nDRK_Power;DRK-Pressure;Diameter\n")
        for item  in mode.listDRK:
            file.write(f"{str(item.drkPower).replace('.',',')};{str(item.drkPressure).replace('.',',')};"
                       f"{str(item.diameter).replace('.',',')}\n")