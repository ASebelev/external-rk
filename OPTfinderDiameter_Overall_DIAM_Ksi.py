...
for mode in fullMode:
    ...
    #listOnlyDiameter = [listPowPress[_i].diameter for _i in range(len(listPowPress))]
...
fig,axs = plt.subplots(2, ceil(len(fullMode) / 2))
for i, mode in enumerate(fullMode):
    ...
    fulltext = []
    ksi = []
    # with open(f'{globalResultDir}/Freq_{mode.freq}_Diam.csv', 'w') as file:
    ...
    for currentMAP in mode.mapData:
        ...
        fulltext.append(f'{currentMAP.diameter}_{currentMAP.ksi}')
        #plt.title(f'Freq {mode.freq}')
        ...
        #     file.write(f"{str(item.drkPower).replace('.',',')};{str(item.drkPressure).replace('.',',')};"
        #                f"{str(item.diameter).replace('.',',')};{str(item.ksi).replace('.', ',')}\n")
    if i < ceil(len(fullMode) / 2):
        axs[0, i].plot(xCoordOrig, yCoordOrig, label='MAP', color='red')
        axs[0, i].plot(xCoordDRK, yCoordDRK, label='DRK', color='black')
        axs[0, i].set_title(mode.freq)
        for x, y, f in zip(xCoordDRK, yCoordDRK, fulltext):
            axs[0, i].text(x, y, f)
    else:
        _i = i - ceil(len(fullMode) / 2)
        axs[1, _i].plot(xCoordOrig, yCoordOrig, label='MAP', color='red')
        axs[1, _i].plot(xCoordDRK, yCoordDRK, label='DRK', color='black')
        axs[1, _i].set_title(mode.freq)
        for x,y,f in zip(xCoordDRK, yCoordDRK, fulltext):
            axs[1, _i].text(x, y, f)

    #plt.legend()
for ax in axs.flat:
    ax.set(xlabel='Pressure', ylabel='Power')
plt.show()