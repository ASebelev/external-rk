...
for currentFreq in listFreq:
    for currentAlt in listAlt:
        for currentTemp in listTemp:
            ...
            #resDir = f'{globalResultDir}/FromServ_freq({currentFreq})_Alt({currentAlt})_Temp({currentTemp}_InPress({currentInPressure})_OutPressLosses({currentOutPressure}))'
            resDir = f'{globalResultDir}/FromServ_alpha({currentFreq})_OutPressLosses({currentAlt}))'
            if RUN_MODE is None:
                os.mkdir(resDir)
