...
endSpeed = 250.0
sizeStepSpeed = 50.0
listSpeed = getVarList(startSpeed, endSpeed, sizeStepSpeed)

# startInPressureRecover = 0.99 # Коэфф. восстан. полн. давл. во вход. устройстве
# endInPressureRecover = 0.89
# sizeInPressureRecover = -0.01
# listInPressureRecover = getVarList(startInPressureRecover, endInPressureRecover, sizeInPressureRecover)

# startOutPressureLosses = 0.01 # Потери в выпускном коллекторе
# endOutPressureLosses = 0.08
# sizeOutStepPressureLosses = 0.01
# listOutPressureLosses = getVarList(startOutPressureLosses, endOutPressureLosses, sizeOutStepPressureLosses)
#
# startAlpha = 0.75
# endAlpha = 0.9
# sizeStepAlpha = 0.01
# listAlpha = getVarList(startAlpha, endAlpha, sizeStepAlpha)
...
for i, currentFreq in enumerate(listFreq):
    for currentSpeed in listSpeed:
        currentTemp = listTemp[0]
            ...
            for key in indexes:
                for num in range(7, len(resText) + 1):
                    ...
                    if key in string:
                        ...
                        break
            variables[0].append(str(currentAlt))
            variables[1].append(str(currentSpeed))
...
with open(f'{globalResultDir}/out.csv', 'w') as file:
    ...
    file.write('\n')
    #file.write(f"Н_ур.мор;{';'.join(variables[0]).replace('.', ',')}\n")
    #file.write(f"dP_In;{';'.join(variables[1]).replace('.', ',')}\n")
