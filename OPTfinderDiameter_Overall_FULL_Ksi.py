...
for mode in fullMode:
    ...
    selectedKSI.append(selectKSI)

selectedKSI = list(set(selectedKSI))
dictKSI = dict()
for mode in fullMode:
    listPowPress = []
    listOnlyPower = []
    ...
    for currentKsi in selectedKSI:
        for currentDiameter in listDiameter:
            ...
            try:
                ...
                listOnlyPower.append(_ne)
                listOnlyPressure.append(_ps)
                listOnlyDiameter.append(currentDiameter)
        ...
        tmp = []
        if currentKsi not in dictKSI.keys():
            dictKSI[currentKsi] = {}
        if mode.freq not in dictKSI[currentKsi].keys():
            dictKSI[currentKsi][mode.freq] = []

        for currentMAP in mode.mapData:
            _index, _diffPwr, _diffPrs = NearestSearch(targetPower=currentMAP.power,
                                                       targetPressure=currentMAP.pressure,
                                                       listPower=listOnlyPower, listPressure=listOnlyPressure)
            #  3 - разница мощностей 4 - разница давлений, текущее кси, diameter
            tmp.append([_diffPwr, _diffPrs, currentKsi, listOnlyDiameter[_index]])

        different = [sum(row[0] for row in tmp), sum(row[1] for row in tmp)]

        # diffPower, diffPress, power, press, diameter
        dictKSI[currentKsi][mode.freq] = different
    pass
pass
minAveragePowerFull = 999999999
minAveragePressureFull = 9999999999

generalKSI = 0
averageDict = {}
for currentKsi, currentDict in dictKSI.items():
    averagePower = sum(currentDict[var][0] / len(currentDict) for var in currentDict.keys())
    averagePressure = sum(currentDict[var][1] / len(currentDict) for var in currentDict.keys())
    print(f'KSI: {currentKsi}; averagePower: {averagePower}; averagePressure: {averagePressure}')
    if averagePower < minAveragePowerFull and averagePressure < minAveragePressureFull:
        generalKSI = currentKsi
        minAveragePowerFull = averagePower
        minAveragePressureFull = averagePressure
...
for mode in fullMode:
    ...
    listOnlyDiameter = []
    for currentDiameter in listDiameter:
        resDir = f'{globalResultDir}/freq({mode.freq})_Diameter({currentDiameter})_Ksi({generalKSI}))'
        if mode.freq == 2100:
            pass
        try:
            resFile = glob(resDir + '/*.res')[0]
    ...
    for currentMAP in mode.mapData:
        _index, _diffPwr, _diffPrs = NearestSearch(targetPower=currentMAP.power, targetPressure=currentMAP.pressure,
                                                   listPower=listOnlyPower,
                                                   listPressure=listOnlyPressure)
        currentMAP.drkPower = listOnlyPower[_index]
        currentMAP.drkPressure = listOnlyPressure[_index]
        currentMAP.diameter = listOnlyDiameter[_index]
        listOnlyPower = listOnlyPower[_index:]
        listOnlyPressure = listOnlyPressure[_index:]
        listOnlyDiameter = listOnlyDiameter[_index:]
        currentMAP.ksi = generalKSI
...
for mode in fullMode:
    ...
    plt.title(f'General ksi: {generalKSI}')
    plt.xlabel('Pressure')
    plt.ylabel('Power')
    xCoordOrig = []
    xCoordDRK = []
    ...
    yCoordDRK = []
    diameter = []
    with open(f'{globalResultDir}/Freq_{mode.freq}_generalKSI.csv', 'w') as file:
